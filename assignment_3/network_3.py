        ...
        for i in range(len(self.in_intf_L)):
            ...
            try:
                ...
                if pkt_S is not None:
                    ...
                    address = int((packet.to_byte_S())[0:5])
                    try:
                        out = self.routing_table[address]
                    except:
                        print("%s: No connection to %i" % (self, address))
                        break
                    self.out_intf_L[out].put(packet.to_byte_S(), True)
                    print('%s: forwarding packet "%s" from interface %d to %d with mtu %d' \
                          % (self, packet, i, i, self.out_intf_L[out].mtu))
            except queue.Full:
                print('%s: The packet "%s" has been lost on interface %d' % (self, packet, i))
                pass
